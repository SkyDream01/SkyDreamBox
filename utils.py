...
def resource_path(relative_path: str) -> str:
    ...
    支持 PyInstaller 和 Nuitka 打包后的环境，也不依赖启动时的工作目录。
    ...
    if getattr(sys, "frozen", False):
        # PyInstaller 使用 _MEIPASS，Nuitka 的资源位于可执行文件所在目录。
...
def format_media_info(data: Dict[str, Any]) -> str:
    ...
    def _safe_get_float(
        data_dict: Dict[str, Any], key: str, default: float = 0.0
    ) -> float:
        """安全获取浮点数值"""
        try:
    ...
    try:
        ...
        if not streams:
            ...
            for stream in streams:
                ...
                info_parts.append(
                    f"<b>{stream_type.capitalize()} #{stream.get('index', '?')}:</b><br>"
                )
                ...
                if stream_type == "video":
                    ...
                    channel_layout = html.escape(
                        str(stream.get("channel_layout", "N/A"))
                    )
