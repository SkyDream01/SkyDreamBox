...
天梦工具箱是一个基于 Python、PySide6 和 FFmpeg 构建的模块化图形用户界面（GUI）工具。它旨在为 FFmpeg 强大的音视频处理功能提供一个直观、易于使用的操作界面，让用户无需记忆复杂的命令行参数即可完成常用的音视频处理任务。
...
天梦工具箱提供了灵活的配置选项，让您可以根据需要自定义工具行为。

### 设置选项卡
- **FFmpeg路径配置**: 如果FFmpeg不在系统PATH中，或您想使用特定版本的FFmpeg，可以在设置中指定自定义路径
- **语言选择**: 支持简体中文和英文界面（需要重启应用生效）
- **主题选择**: 深色/浅色主题切换
- **其他选项**: 自动检查更新、文件覆盖行为等

### 配置文件
配置保存在 `config/config.json` 文件中，位于应用目录下。您可以直接编辑该文件，或通过程序内的设置界面修改。
