...
class Workbench(QMainWindow):
    ...
    def _build(self):
        ...
        side.addWidget(note("本地处理\n单个文件 · 批量任务"))
        layout.addWidget(sidebar)
        right = QWidget()
        body = QVBoxLayout(right)
        body.setContentsMargins(22, 18, 22, 12)
        header = QHBoxLayout()
        self.title = QLabel()
        ...
        files_layout.setContentsMargins(0, 0, 12, 0)
        actions = QHBoxLayout()
        ...
        files_layout.addWidget(note("拖入文件开始；点击文件检查轨道与配对，统一参数批量入队。"))
        ...
        self.horizontal.addWidget(files_panel)
        editor_panel = QWidget()
        editor_layout = QVBoxLayout(editor_panel)
        editor_layout.setContentsMargins(10, 0, 0, 0)
        presets = QHBoxLayout()
        presets.addWidget(QLabel("任务参数"))
        presets.addStretch()
